...
export default function UploadScreenV4({ onDataParsed }: UploadScreenProps) {
                ...
                <div className="w-[72px] shrink-0 flex flex-col gap-3 border-r border-on-surface/10 pr-2">
                  {(["npm", "web", "claude"] as const).map((t) => (
                    ...
                    >
                      {activeTab === t && (
                        <motion.div
                          layoutId="v4-left-border"
                          className="absolute left-[-2px] top-2 bottom-2 w-[3px] rounded-r-md bg-primary"
                        />
                      )}
                      {t === "npm" ? (
                        <svg
