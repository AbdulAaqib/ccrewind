    ...
    <div className="flex flex-col gap-1.5 w-full">
      {top.map((t) => (
        <div key={t.name} className="flex items-center gap-2">
          <span className="font-label text-[8px] md:text-[9px] uppercase tracking-wider text-on-surface/40 w-10 md:w-12 text-right shrink-0">{t.name}</span>
    ...
    <div className="w-full min-h-screen flex flex-col items-center justify-center px-4 md:px-8 py-12 md:py-20">
      ...
      >
        ...
        <div className="grid grid-cols-4 divide-x divide-[#3d3d3a]/30 border-b border-[#3d3d3a]/30">
          ...
          <StatCell label="Days" value={stats.totalActiveDays.toString()} sub={`${stats.longestStreak}d streak`} delay={0.45} />
        </div>
            ...
            {stats.topProjectStats.slice(0, 3).map((proj, i) => {
              ...
              return (
                <div key={proj.name} className="flex items-center gap-2">
                  <span className="font-label text-[8px] md:text-[9px] text-on-surface/40 w-16 md:w-20 text-right shrink-0 truncate">{name}</span>
                  <div className="flex-1 h-3 md:h-3.5 bg-[#2f2f2d] rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
        ...
        <div className="border-t border-[#3d3d3a]/30 p-3 md:p-4">
          ...
          <div className="flex justify-between">
            <div className="flex items-center gap-1">
              ...
              <span className="font-label text-[7px] md:text-[8px] text-on-surface/40">Input {fmt(stats.totalInputTokens)}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-[#ffb59d]" />
              <span className="font-label text-[7px] md:text-[8px] text-on-surface/40">Output {fmt(stats.totalOutputTokens)}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-[#ffdbd0]" />
              <span className="font-label text-[7px] md:text-[8px] text-on-surface/40">Cache {fmt(stats.totalCacheReadTokens)}</span>
            </div>
          </div>
        </div>
      </motion.div>
