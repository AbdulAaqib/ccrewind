...
export default function Arsenal({ stats }: { stats: ComputedStats }) {
  ...
  useEffect(() => {
    ...
    tools.forEach((tool, i) => {
      ...
      g.append("text")
        ...
        .attr("opacity", 0)
        .text(tool.count.toLocaleString())
        .transition()
        .delay(600 + i * 100)
        .duration(400)
        .attr("opacity", 1);
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-4 md:px-6 py-12 md:py-20 max-w-2xl mx-auto">
      ...
      >
        <img src="/mascots/arsenal.png" alt="Arsenal mascot" className="w-full h-full object-cover" />
      </motion.div>
      <motion.div
      ...
      >
        ...
        <span className="font-label text-[10px] uppercase tracking-widest text-on-surface/40 ml-2">
          {narrative.statLabel}
        </span>
      </motion.div>
    </div>
