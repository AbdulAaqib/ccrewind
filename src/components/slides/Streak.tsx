...
export default function Streak({ stats }: { stats: ComputedStats }) {
  ...
  useEffect(() => {
    ...
    // Ensure every active date has at least some activity so it renders with color
    for (const date of dates) {
      if (!activityMap.has(date)) {
        // Use a seeded pseudo-random value based on date for consistent varied display
        const seed = date.split("-").reduce((a, b) => a + parseInt(b), 0);
        activityMap.set(date, 1 + (seed % 5));
      }
    }
    ...
    const width = cols * total + 20,
      height = rows * total + 20;
    svg.attr("viewBox", `0 0 ${width} ${height}`);

    const maxActivity = Math.max(...displayDates.map((d) => activityMap.get(d) || 1), 1);
    const colorScale = d3
      .scaleSequential(d3.interpolateRgbBasis(["#383835", "#ff6b35", "#ffdbd0"]))
    ...
    displayDates.forEach((date, idx) => {
      ...
      g.append("rect")
        ...
        .attr("opacity", 1)
        .attr("fill", dateSet.has(date) ? colorScale(activity) : "#2f2f2d");
    });
  }, [stats]);
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-4 md:px-6 py-12 md:py-20 max-w-2xl mx-auto">
      ...
      >
        <img src="/mascots/streak.png" alt="Streak mascot" className="w-full h-full object-cover" />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
